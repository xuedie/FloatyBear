    ...
    public class PlacementExample : MonoBehaviour
    {
        ...
        private void HandlePlacementComplete(Vector3 position, Quaternion rotation)
        {
            if (_placementPrefabs != null && _placementPrefabs.Length > _placementIndex)
            {
                ...
                Vector3 scale = playground.transform.localScale;
                playground.transform.position = position + new Vector3(0, scale.y, 0) + offset;
                playground.transform.rotation = rotation;
                playground.gameObject.SetActive(true);

                mapper.DestroyAllMeshes();
                Destroy(_placementObject.gameObject);
                mainController.OnPlacementFinished();
